...
- 2026-06-07: Center migrated from Cloud APIs → local **Command-R** (`cohereforai.c4ai-command-r-08-2024`) in LM Studio, slot :3. Cloud APIs removed from the active surface; no fallback configured.
- 2026-06-07: Tool Agent (formerly) role **absorbed** into Center. The fourth voice (Command-R on slot :3 for Cline/tool work) is now just Center. `personas/tool-agent/` moved to `personas/.deprecated/tool-agent/` for history. `scripts/tool-c.fish` renamed/repurposed to `scripts/center-c.fish` and now points to Center's inbox.
