...
`config/grok-lmstudio.toml.example`).
...
> in LM Studio on slot :3. There is no cloud Grok fallback.
> **Tool Agent (the previous fourth voice) is gone** — its role is
...
- **Trinity is now three voices** — Left, Right, Center. The previous
  Tool Agent (a separate Command-R slot) is absorbed into Center.
...
- `config/grok-lmstudio.toml.example` (the `[model.*]` sections)
