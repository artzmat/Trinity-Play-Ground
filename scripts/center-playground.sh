#!/usr/bin/env bash
# Center Playground / Center Launcher
...
Center / Center — main PCaC control + controlled chill layer (monitor: $PCAC_CENTER_MONITOR).
...
Options:
  ...
  --watch-left [USER]  Open full Left screen (DP-3): tmux watch (health/logs) + chat box for Left-Brain persona.
                       'center: ... in chat to query Center. Supports remote user labels.
  --watch-right [USER] Open full Right screen (DP-2): tmux watch (git) + chat box for Right-Brain persona.
                       'center: ... in chat to query Center. Supports remote user labels.
...
main() {
  ...
  pcac_log INFO "Purpose: Orchestrator + controlled chill layer + interface"
  pcac_show_env
  ...
  pcac_log INFO "Next steps for Center: orchestration, Center UI surface, monitor routing, state machine."
