#!/usr/bin/env bash
# Left TMUX session: combined watch (top) + full Grok TUI as Left-Brain persona (bottom).
# This turns the Left monitor (DP-3) into a self-contained "Left persona" terminal
# with monitoring + a full powerful Grok CLI running as the Left-Brain (cloud, persona-injected).
...
# Split horizontal (bottom pane for full Grok TUI as Left-Brain persona)
...
tmux split-window -v -t "$SESSION" -p 30 -c "$SCRIPT_DIR" \
  "bash -c 'source \"$SCRIPT_DIR/lib/common.sh\"; pcac_run_grok_persona left \"${1:-$(whoami)}\"'"
