#!/usr/bin/env bash
# Right TMUX session: combined watch (top) + full brain TUI as Right-Brain persona (bottom).
# This turns the Right monitor (DP-2) into a self-contained "Right persona" terminal
# with monitoring + a full powerful LM Studio CLI running as the Right-Brain (cloud, persona-injected).
# No LM Studio needed.
...
# Split horizontal (bottom pane for full brain TUI as Right-Brain persona)
# Full grok cli as the creative/playful persona (no local LM required).
tmux split-window -v -t "$SESSION" -p 30 -c "$SCRIPT_DIR" \
  "bash -c 'source \"$SCRIPT_DIR/lib/common.sh\"; pcac_run_brain_persona right \"${1:-$(whoami)}\"'"
