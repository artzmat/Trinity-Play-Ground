#!/usr/bin/env bash
# Right Chat Box - for the Right-Brain persona
# Separate chat from Left.
# 'center: your question' now uses local LMStudio Qwen as the "Right Grok" persona.
# 'center: your question' sends to main Center cli (posts as center_query).
...
show_chat() {
  clear
  echo "╔════════════════════════════════════════════════════════════╗"
  echo "║  RIGHT CHAT BOX  |  Right-Brain persona  |  User: $USER_LABEL  ║"
  echo "╚════════════════════════════════════════════════════════════╝"
  echo "This is the separate chat for Right. Center sees this."
  echo "You (Right) can view the Center monitor but have no control over it."
  echo "Type 'center: ... to ask the local Right-Brain (LM Studio Qwen as Right-Brain)."
  echo "Type 'center: ...' to ask the Center (main grok cli)."
  ...
  case "$input" in
    ...
    grok:*|Grok:*)
      ...
      if pcac_ask_brain right "$msg" "$USER_LABEL"; then
        echo "[$(date '+%H:%M:%S')] Right-Brain (as Right-Brain) replied (see chat log)"
      else
