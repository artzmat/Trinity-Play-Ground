#!/usr/bin/env bash
# Left Chat Box - for the Left-Brain persona
# Separate chat from Right.
# 'center: your question' now uses local LMStudio Qwen as the "Left Grok" persona.
...
show_chat() {
  clear
  echo "╔════════════════════════════════════════════════════════════╗"
  echo "║  LEFT CHAT BOX  |  Left-Brain persona  |  User: $USER_LABEL   ║"
  echo "╚════════════════════════════════════════════════════════════╝"
  echo "This is the separate chat for Left. Center sees this."
  echo "You (Left) can view the Center monitor but have no control over it."
  echo "Type 'center: ... to ask the local Left-Brain (LM Studio Qwen as Left-Brain)."
  ...
  case "$input" in
    ...
    grok:*|Grok:*)
      ...
      if pcac_ask_brain left "$msg" "$USER_LABEL"; then
        echo "[$(date '+%H:%M:%S')] Left-Brain (as Left-Brain) replied (see chat log)"
      else
