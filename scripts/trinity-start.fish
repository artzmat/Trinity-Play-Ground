...
#
# Launches the small/minimized konsole + tmux + brain persona TUI windows:
...
#   Left: interact with Left-Brain directly in its TUI (or via its chat log).
#   Right: interact with Right-Brain.
#   Center: use the monitor window + your main desktop + composer + online online chat .
#
# To capture the current CLI outputs (left + right + center + trinity synthesis) for online chat:
...
if test (count $argv) -gt 0; and contains -- $argv[1] --help -h
    ...
    echo "Opens small (960x600) minimized/floating konsole windows with tmux + the full cloud API TUI"
...
echo "  • Left screen:  PCaC-Left-Screen-...  (top: watch, bottom: Left-Brain CLI TUI)"
echo "  • Center screen: PCaC-Center-Monitor-Both-... (splits for Left+Right chat tails + bus)"
echo "  • Right screen: PCaC-Right-Screen-... (top: watch, bottom: Right-Brain CLI TUI)"
echo ""
echo "Tips:"
...
echo "  - To capture the current Left/Right (and Center) CLI outputs + Trinity synthesis for online chat:"
echo "      trinity-copy"
