...
# for easy paste or file upload into online chat (online chat).
#
...
#
# "To get left and right CLI logs for online chat" — exactly. The pane captures pull
# directly from the running brain TUI sessions on Left and Right screens.
...
if test (count $argv) -gt 0; and contains -- $argv[1] --help -h
    echo "trinity-copy — Capture Left + Right (and Center) CLI logs for online chat."
    ...
    echo "Primary fish command for \"Trinity Copy To get left and right CLI logs for online chat.\""
    exit 0
end
