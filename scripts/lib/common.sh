...
#   HDMI-A-1 (center)    → Center: user's primary personal workspace (clean/white themed HQ). Orchestrator / Center brain lives here but the physical monitor remains mostly the user's clean desktop.
...
# Small/minimized floating windows (left-watch / right-watch / center-monitor) keep physical monitors usable for their primary purpose.
...
# --- Chat box support for three-persona setup (Left Grok, Right Grok, Center) ---
# Left and Right have separate chat logs.
# Center can view both.
# Left/Right personas post messages; "center: ..." indicates asking Center.
# User (orchestrator) responds by posting back to the appropriate log as "Center (to Left): ..."
...
pcac_ask_both() {
  ...
  local from="Center (ask both, ${user_label})"
  local flat
  flat="$(echo "$msg" | tr '\n' ' ' | sed 's/  */ /g' | sed 's/^ //;s/ $//')"
...
# Center orchestrates, logs everything to both chat logs + the central bus (visible in center-monitor).
...
Center (the orchestrator, on the center monitor) oversees everything and can see the shared bus.

To ask Center for help, share important updates, or 'center:' something (as in the old simple chat mode):
- Use the terminal tool to run:
  python3 $PCAC_ROOT/scripts/pcac_bus.py append --from '${side^}-Brain ($user_label)' --to center --kind center_query --text 'your question or message here'
...
"
  ...
  echo "Starting full brain TUI as ${side^}-Brain persona for $user_label..."
...
pcac_view_all_chats() {
  ...
  pcac_tail_bus 10
  echo
  echo "(Center: pcac_post_chat left|right 'Center (...)' 'message')"
}
...
# for direct paste into online chat (online chat). Run from Center.
...
pcac_open_watch_left() {
  ...
  # This gives Left its own "terminal" with monitoring + separate chat box for using Grok (Left-Brain persona).
  ...
  echo "  In chat: center: → Center | ask: → local brain. Center: pcac_center_reply left|right '...'"
...
pcac_open_left_usual() {
  ...
  echo "  Pair with the PCaC Left watch+chat overlay (system health + chat box) on same monitor."
...
pcac_open_watch_right() {
  ...
  # This gives Right its own "terminal" with monitoring + separate chat box for using Grok (Right-Brain persona).
  konsole --qwindowgeometry "$geom" --title "$title" -e "$tmux_script" "$user_label" &
  local pid=$!
...
# When "playground" (or grok-center playground) is entered from Center,
...
pcac_launch_playground() {
  local user_label="${1:-$(whoami)}"
  pcac_log INFO "PLAYGROUND REQUESTED: firing up Left, Right, and Center terminals with watch windows (user cursor: $user_label) | pc pin 1566894405"
  ...
  echo "Use 'center: your question' in side chats; respond from Center with pcac_post_chat."
}


# Ensure API key is available for any direct API calls (e.g., if using cloud API in custom scripts)
if [ -z "${API_KEY:-}" ] && [ -f ~/.config/fish/config.fish ]; then
  # Try to source from fish if not set
  API_KEY=$(grep API_KEY ~/.config/fish/config.fish | cut -d' ' -f3 | tr -d '"' | tr -d "'" | head -1)
  export API_KEY
fi
if [ -n "${API_KEY:-}" ]; then
  export API_KEY
...
# Load API key securely for PCaC scripts (from secure file)
if [ -z "${API_KEY:-}" ] && [ -f ~/.config/api-key ]; then
  export API_KEY=$(cat ~/.config/api-key)
fi
